...
def main(config,i_fold):
    ...
    if config.model_type == 'swin':
        ...
        model = GMS_3DQA(pretrained = False)
    ...
    if config.database == 'SJTU':
        train_filename_list = 'datainfo/sjtu_data_info/train_' + str(i_fold) + '.csv'
        test_filename_list = 'datainfo/sjtu_data_info/test_' + str(i_fold) + '.csv'
    elif config.database == 'WPC':
        train_filename_list = 'datainfo/wpc_data_info/train_' + str(i_fold) + '.csv'
        test_filename_list = 'datainfo/wpc_data_info/test_' + str(i_fold) + '.csv'
    elif config.database == 'WPC2.0':
        train_filename_list = 'csvfiles/wpc2.0_data_info/train_' + str(i_fold) + '.csv'
        test_filename_list = 'csvfiles/wpc2.0_data_info/test_' + str(i_fold) + '.csv'


    # datainfo = os.path.join('datainfo',config.database,config.database + '_datainfo_' + str(i_fold) + '.csv')
    # datainfo = os.path.join('datainfo', 'train' + '.csv')
    images_dir = config.images_dir
    print('using train datainfo: ' + train_filename_list)
    ...
    trainset = QMM_Dataset(csv_file = train_filename_list, data_prefix = images_dir, img_length_read=config.img_length_read)
    testset = QMM_Dataset(csv_file = test_filename_list, data_prefix = images_dir, img_length_read=config.img_length_read)
    ...
    pbar = tqdm(total = len(train_loader) *config.epochs)
    for epoch in range(config.epochs):
        ...
        with torch.no_grad():
            ...
            if test_SRCC > best_test_criterion:               
                ...
                print('Saving model to', os.path.join(config.ckpt_path, config.database + '_fold_' + str(i_fold) + '_best.pth'))
    return best_test
...
if __name__ == '__main__':
    ...
    # input parameters
    parser.add_argument('--database', type=str, default = 'SJTU')
    parser.add_argument('--model_type', type=str, default = 'none')
    parser.add_argument('--num_workers', type=int, default=4)
    parser.add_argument('--images_dir', type=str, default='D:\PCQA\data\sjtu-database\img_6')
