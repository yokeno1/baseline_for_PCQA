...
def parse_args():
    ...
    parser.add_argument('--data_dir_2d', default='D:\PCQA\data\sjtu-database\img', type=str, help = 'path to the images')
    parser.add_argument('--data_dir_pc', default='D:\PCQA\data\sjtu-database\patch', type=str, help = 'path to the patches')
...
if __name__=='__main__':
    ...
    for k_fold_id in range(1,args.k_fold_num + 1):
        ...
        # elif database == 'lrl':
        #     train_filename_list = 'csvfiles/lrl_data_info/train_' + str(k_fold_id) + '.csv'
        #     test_filename_list = 'csvfiles/lrl_data_info/test_' + str(k_fold_id) + '.csv'
    
        transformations_train = transforms.Compose([transforms.RandomCrop(224,pad_if_needed=True, fill=0, padding_mode='constant'),transforms.ToTensor(),\
        ...
        for epoch in range(num_epochs):
            ...
            for i, (imgs, pc, mos) in tqdm(enumerate(train_loader),total=len(train_loader)):
                imgs = imgs.to(device)
            ...
            with torch.no_grad():
                for i, (imgs, pc, mos) in tqdm(enumerate(test_loader),total=len(test_loader)):
                    imgs = imgs.to(device)
                    pc = torch.Tensor(pc.float())
                    pc = pc.to(device)
                    y_test[i] = mos.item()
